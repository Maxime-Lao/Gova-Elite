...
export default function BookingsCard({ rent, user, onDelete, onBookingChange }) {
  ...
  useEffect(() => {
    const fetchRentedTimes = async () => {
      try {
        const response = await fetch(`https://kame-os.fr/api/cars/${rent.car.id}/rents`, {
            method: 'GET',
            headers: {
  ...
  const fetchUpdatedRentedTimes = async (carId) => {
    try {
      const response = await fetch(`https://kame-os.fr/api/cars/${carId}/rents`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
                Authorization: `Bearer ${token}`
  ...
  const handleCancel = async (id) => {
    try {
      const response = await fetch(`https://kame-os.fr/api/rents/${id}`, {
          method: 'DELETE',
          headers: {
              'Content-Type': 'application/json',
              Authorization: `Bearer ${token}`
  ...
  const handleRentalSubmit = async (rentId, newPaymentIntent) => {
    if (startDate && endDate) {
      ...
      try {
        const response = await fetch(`https://kame-os.fr/api/rents/${rentId}`, {
          method: 'PATCH',
          headers: {
  ...
  return (
    <Card sx={{ maxWidth: 345 }}>
      ...
      <CardMedia
        component="img"
        height="194"
        image={rent.car.media.length &&  rent.car.media[0].filePath ? `https://kame-os.fr/media/${rent.car.media[0].filePath}` : "https://source.unsplash.com/random"}
