...
function CarDetails() {
  ...
  useEffect(() => {
    fetch(`https://kame-os.fr/api/cars/${id}/comments`, {
      method: 'GET',
      headers: {
  ...
  useEffect(() => {
    fetch(`https://kame-os.fr/api/cars/${id}`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${token}`
          ...
          <Card sx={cardStyle}>
            <Slider {...getSliderSettings(car.media.length)}>
              {car.media.length > 0 ? car.media.map((media, index) => (
                <div key={index}>
                  <img src={`https://kame-os.fr/media/${media.filePath}`} alt={`Image de voiture ${index + 1}`} style={imageStyle} />
                </div>
              )) : (
                <div>
